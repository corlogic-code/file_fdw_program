 ...
 * SQL functions
 */
PG_FUNCTION_INFO_V1(file_fdw_program_handler);
PG_FUNCTION_INFO_V1(file_fdw_program_validator);
 ...
 */
Datum
file_fdw_program_handler(PG_FUNCTION_ARGS)
{
 ...
 */
Datum
file_fdw_program_validator(PG_FUNCTION_ARGS)
{
	...
	if (catalog == ForeignTableRelationId && filename == NULL && program == NULL)
		ereport(ERROR,
				(errcode(ERRCODE_FDW_DYNAMIC_PARAMETER_VALUE_NEEDED),
				 errmsg("either filename or program is required for file_fdw_program foreign tables")));
